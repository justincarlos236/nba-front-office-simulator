...
- **Season calendar** (`dayIndex`): each `Game` gets a sequential
  ...
  get a day-indexed schedule. The Standings page's `SeasonCalendar`
  component shows "calendar available starting next season" for games
  with no `dayIndex` instead of fabricating dates for old data.
