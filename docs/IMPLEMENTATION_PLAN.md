...
| 13  | NBA Draft                             | ✅     | P1       | 4       | 66,67        | Interactive `/leagues/[id]/draft` - lottery, 60-pick order, generated class, real rookie contracts; CPU picks identity/need/personality-aware (`draftAi.ts`) with in-draft CPU pick trades and real reach/steal news; full broadcast-style redesign (header, order rail, animated reveal, searchable/comparable prospect board, team-needs tab)                                                        |
...
| 46  | AI General Managers                   | 🟡     | P1       | 11      | 21,22        | CPU teams evaluate user-proposed trades (11c), re-sign their own expiring players intelligently (Phase 1), and now initiate their own trades with each other via needs/identity-biased targeting + mutual evaluateTradeOffer acceptance (Phase 2, 2026-07-23) - see docs/ARCHITECTURE.md; CPU free-agent signings are still random-but-cap-legal, not run through the new evaluation - planned Phase 3 |
...
| 66  | Draft Lottery                         | ✅     | P1       | 4       | 17           | `draftLottery.ts` odds engine + full Draft Lottery Experience (`/leagues/[id]/draft/lottery`) - animated pick-by-pick reveal, odds overview, News/Fans/History integration                                                                                                                                                                                                                             |
...
**Phase 1 DONE (2026-07-28)**: persistent `User.gmReputation`, a real firing
at owner-confidence 0 that snapshots a permanent `CareerRecord`, career-
earnings tracking, voluntary retirement, an ended-league read-only recap
(`CareerEndRecap`), and a `/career` page (reputation + title + past tenures).
See `docs/FEATURE_REQUESTS.md`'s GM Career Mode entry. **Phase 2 (not built):
the reputation-gated GM Job Market.** Original design brief below (2026-07-21),
kept for the Phase 2 scope - a full second pillar (persistent, cross-league
identity), not just "what happens when you're fired":
...
### Phase 13 — Franchise Finances & Business Operations

Detailed user brief (2026-07-28, full text in `docs/FEATURE_REQUESTS.md`).
Architecture-overlap review confirmed money's drivers (attendance,
popularity, market, ownership, playoffs, payroll, staff) all already exist,
so this is a **consumer** of them, not a parallel simulation. Cap/CBA stays
authoritative - money never buys cap space. See `docs/ARCHITECTURE.md`'s
"Franchise finances" section.

- [x] **13a — Business ledger & consequence layer** ✅ DONE (2026-07-28):
      revenue/expense/net-income/cash/financial-health/franchise-value model
      (`src/lib/finances/`), a `FinancialSnapshot` per league+team+season, the
      league-wide season P&L pass in `advanceSeasonAction`, an optional
      financial-health nudge to owner confidence, `FINANCIAL_REPORT` /
      `FRANCHISE_MILESTONE` news, a `/finances` dashboard + team-dashboard
      card + nav entry, and a backfill for existing leagues. Levers persist
      but are neutral/read-only in this sub-phase.
- [x] **13b — The two user levers + investment sinks** ✅ DONE (2026-07-28):
      ticket-pricing posture now feeds gate revenue (already) **and** a
      season-boundary fan-happiness delta (`TICKET_POSTURE_FAN_DELTA`);
      facilities investment feeds `developPlayerRating` (a fourth neutral-
      anchored bonus) and medical investment feeds the injury-frequency roll
      in `rollForTeamInjury` (`INVESTMENT_QUALITY_DELTA`). Interactive
      controls on `/finances` (`BusinessStrategyControls` +
      `updateBusinessStrategyAction`, server-validated). CPU teams get a
      market-based ticket posture at bootstrap + backfill
      (`pickCpuTicketPosture`); CPU investment stays STANDARD (Phase 13c).
- [x] **13c — Depth & polish** ✅ DONE (2026-07-28): CPU financial restraint
      (`financialSpendingResistance` raises a cash-strapped CPU team's
      re-signing bar via a new optional `financialThresholdMultiplier` on
      `evaluateReSigningDecision` - salary-normalized scoring means it only
      cuts expensive marginal retentions, never bargains, so it never
      cripples); a profit/loss-by-season chart on `/finances`
      (`NetIncomeHistoryChart`) beside the franchise-value trend. **CPU
      investment into the development/injury systems is deliberately held at
      neutral** - letting big-market CPU teams buy PREMIUM facilities would
      compound a hard-to-balance development edge over many seasons for little
      user-visible payoff, so CPU financial participation stays at
      revenue/expenses/value/ticket-posture/spending-restraint. Franchise-
      value billion-crossing milestones from 13a already surface value
      history; no further milestone types were needed.

- [x] **13d — Strategic depth pass** ✅ DONE (2026-07-28): after an agreed
      weakness review (money bit when struggling, went inert when winning).
      Emergent owner tax tolerance + escalating loss mandate via a multi-season
      `FinancialStanding` on the existing owner-confidence system
      (`ownershipFinance.ts`, `League.financialMandateSeason`); a derived
      franchise-icon model (`franchiseIcon.ts`, `LeaguePlayer.joinedTeamSeason`
      / `homegrown`) feeding franchise value + a scaled "losing an icon" trade
      hit; mid-season P&L projection, ownership-standing + mandate UI,
      investment ROI cost labels, and a profile icon badge. Cap/CBA authority
      preserved; the solution to successful-team cash accumulation is
      trust-buys-runway, not a spending sink.
