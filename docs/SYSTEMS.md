...
leagues (anything that instead _persists_ new state has to be explicitly
backfilled onto every existing league, not just wired into
`createLeagueAction` - see `scripts/README.md`).
...
A fresh, extensive user request . Explicitly invoked the architecture-overlap-review protocol
...
## News page: filtering, search and awards
...
## CPU autonomous GM intelligence
...
A fresh, out-of-band user request . Preceded by an architecture-overlap
...
A fresh, out-of-band user request . Preceded by an architecture-overlap review the user explicitly
...
A further, more detailed request to make `fanHappiness` a _living_ number
...
A fresh, out-of-band user request . Preceded by an
...
- **Retirement** (`retirement.ts`): zero risk below 33, ~8%/year after
  that (higher for already-low-rated players), forced at 41. The rate was
  set while there was still no draft, so that retirement could not drain a
  league it had no way to refill; the draft has since shipped, and the
  20-season harness in `src/lib/development/longSave.invariant.test.ts`
  now checks intake and attrition actually balance.
...
## AI GM assistant — not built

**This is a design sketch, not a description of shipped code.** Only the
`AssistantThread` / `AssistantMessage` models exist. There is no chat UI, no
tool-use wiring, and no model call anywhere in the codebase. It is recorded
here because the design shaped a decision that _did_ ship, described below.

The intended shape: not a chat window that free-associates about basketball,
but a model given tool-use against the same domain functions the UI already
calls — team cap sheet, player valuation, trade validation — so that "can I
afford to extend this guy" is answered from that league's real state rather
than from a plausible-sounding guess.

What that requirement actually produced: **quantitative player valuation was
built as a standalone model** rather than as assistant plumbing. Surplus
value (projected on-court production value minus cap hit, age-curve
adjusted) powers trade grades, the CPU trade AI, and contract generation
today, with no LLM involved. The assistant would have been a consumer of it,
not its owner — which is why the valuation engine shipped and the assistant
did not.
...
- **Existing leagues were backfilled**, not left on the old scale: a
  ...
  discipline as every other persisted-state migration - see
  `scripts/README.md`.
- **Minutes-normalization fix** (found via a user bug report - a fresh
  ...
  mapping, not just its scale). This remains a hand-tuned
...
> TS% inputs (no BPM/VORP). The notes below are retained as historical
> context for the original 2023-24 approach.
...
## In-league navigation & phase model
...
nav) with the Action Center as a deliberate, separate follow-up.
