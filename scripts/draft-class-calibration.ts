/**
 * Fits `POTENTIAL_FALLOFF_EXPONENT`.
 *
 * Attempt five. Attempts 1-4 all lived in developPlayerRating and all moved
 * every talent band together; attempt 4 established why - in this model the
 * league's stars come almost entirely from the draft, so "top picks can fail"
 * and "the league has stars" are the same knob.
 *
 * So this one changes the intake instead. DRAFT_AUDIT D-P2-1: classes carry 80+
 * ceilings at 42.8% against a league at 28.2%. A steeper potential falloff
 * leaves genuine star ceilings at the very top of a class and a long tail
 * behind them, which is what a real draft looks like and is the only shape that
 * can deliver busts and stars at once.
 *
 ...
 *
 * Calls the real `generateDraftClass` through its `falloffExponent` seam, so
 * this measures the shipped generator rather than a copy of it. Results are
 * averaged over five twenty-season runs - a single run swings 85+ by about
 * five, which is larger than the gap between adjacent candidates.
 *
 * Run: npx tsx scripts/draft-class-calibration.ts
 */
...
function simulateLeagueOnce(exponent: number, seed: number) {
  ...
  for (let season = 1; season <= SEASONS; season++) {
    ...
    classCounter += 1;
    for (const [i, prospect] of generateDraftClass(rng, exponent).prospects.entries()) {
      league.push({
...
function simulateProspects(exponent: number) {
  const rng = makeRng(555);
  let pick1Bust = 0;
  let pick1Reached80 = 0;
  const PICK1_TRIALS = 1500;
  for (let t = 0; t < PICK1_TRIALS; t++) {
    const prospect = generateDraftClass(rng, exponent).prospects[0];
    const id = `p1:${t}`;
  ...
  for (let t = 0; t < CLASS_TRIALS; t++) {
    for (const [i, prospect] of generateDraftClass(classRng, exponent).prospects.entries()) {
...
console.log("DRAFT CLASS POTENTIAL FALLOFF CALIBRATION");
console.log("=".repeat(86));
console.log(
...
console.log(
  `${"EXPONENT".padStart(9)}${"P1 BUST".padStart(10)}${"P1 -> 80+".padStart(11)}${"YIELD".padStart(8)}` +
    `${"90+".padStart(7)}${"85+".padStart(7)}${"80+".padStart(7)}${"ERR".padStart(9)}`,
);

let best = { exponent: 0, err: Infinity };
for (let exponent = 0.15; exponent <= 0.7001; exponent += 0.05) {
  const league = simulateLeague(exponent);
  const prospects = simulateProspects(exponent);
  const err =
  ...
  if (err < best.err) best = { exponent, err };
  console.log(
    `${exponent.toFixed(2).padStart(7)}${((prospects.pick1Bust * 100).toFixed(1) + "%").padStart(10)}` +
