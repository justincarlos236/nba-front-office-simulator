...
import { contractQualityScore, priceContractCents, pickContractLength } from "../src/lib/contracts/priceContract";
...
interface Priced {
  name: string; team: string; pos: string; ovr: number; pot: number;
  age: number; exp: number; salary: number; years: number; tier: string; gp: number;
}
...
for (const [label, v] of [
  ["minimum", Math.min(...salaries)], ["median", pctile(salaries, 0.5)],
  ["mean", mean(salaries)], ["75th", pctile(salaries, 0.75)],
  ["90th", pctile(salaries, 0.9)], ["95th", pctile(salaries, 0.95)],
...
console.log(`  average per team      ${usd(totalPayroll / 30)}   (cap ${usd(cap)}, ${((totalPayroll / 30 / cap) * 100).toFixed(0)}% of it)`);

console.log(`\n  TOP 15 PAID`);
console.log(`  ${"PLAYER".padEnd(24)}${"OVR".padStart(4)}${"AGE".padStart(4)}${"POS".padStart(5)}${"SALARY".padStart(9)}${"%CAP".padStart(7)}`);
for (const p of [...league].sort((a, b) => b.salary - a.salary).slice(0, 15))
  console.log(`  ${p.name.slice(0, 22).padEnd(24)}${String(p.ovr).padStart(4)}${String(p.age).padStart(4)}${p.pos.padStart(5)}${usd(p.salary).padStart(9)}${pctCap(p.salary).padStart(7)}`);

// --------------------------------------------------------------- 4. BY TIER
h("4. SALARY BY QUALITY TIER");
const TIERS = ["SUPERSTAR", "STAR", "STARTER", "ROTATION", "MINIMUM"] as const;
console.log(`  ${"TIER".padEnd(11)}${"N".padStart(5)}${"MIN".padStart(9)}${"MEDIAN".padStart(9)}${"MEAN".padStart(9)}${"MAX".padStart(9)}${"%CAP MED".padStart(10)}`);
...
for (const t of TIERS) {
  ...
  console.log(`  ${t.padEnd(11)}${String(g.length).padStart(5)}${usd(Math.min(...s)).padStart(9)}${usd(pctile(s, 0.5)).padStart(9)}${usd(mean(s)).padStart(9)}${usd(Math.max(...s)).padStart(9)}${pctCap(pctile(s, 0.5)).padStart(10)}`);
}
...
for (let i = 1; i < TIERS.length; i++) {
  ...
  console.log(`    ${TIERS[i].padEnd(10)} max ${usd(Math.max(...lo)).padStart(8)} vs ${TIERS[i - 1].padEnd(10)} median ${usd(pctile(hi, 0.5)).padStart(8)}  ${overlap ? "OVERLAP" : "ok"}`);
}
...
for (const pot of [78, 85, 92, 99]) {
  const s = priceContractCents({
    season: SEASON, quality: contractQualityScore({ overallRating: 78, performanceScore: null, gamesPlayed: 0 }),
    age: 24, yearsOfExperience: 4, position: "SF",
  });
...
console.log(`  ${"AGE".padStart(5)}${"SVC".padStart(5)}${"SALARY".padStart(10)}${"%CAP".padStart(8)}${"YEARS".padStart(7)}${"TOTAL".padStart(10)}${"MAX TIER".padStart(10)}`);
for (const age of [20, 23, 26, 29, 32, 35, 38]) {
  ...
  const s = priceContractCents({ season: SEASON, quality: q, age, yearsOfExperience: exp, position: "SF" });
  const y = pickContractLength(q, age, createSeededRandom(`age${age}`));
  console.log(`  ${String(age).padStart(5)}${String(exp).padStart(5)}${usd(s).padStart(10)}${pctCap(s).padStart(8)}${String(y).padStart(7)}${usd(s * y).padStart(10)}${(maxSalaryFractionFor({ age, yearsOfExperience: exp }) * 100).toFixed(0).padStart(9)}%`);
}

// --------------------------------------------------------------- 7. LENGTH
h("7. CONTRACT LENGTH BY ARCHETYPE");
console.log(`  ${"ARCHETYPE".padEnd(24)}${"OVR".padStart(5)}${"AGE".padStart(5)}${"SALARY".padStart(10)}${"YEARS (10 seeds)".padStart(20)}`);
for (const [name, ovr, age] of [
  ["superstar", 95, 27], ["star", 88, 28], ["starter", 79, 27], ["role player", 72, 27],
  ["fringe", 65, 27], ["young prospect", 70, 21], ["aging veteran", 78, 35], ["old star", 88, 36],
] as [string, number, number][]) {
  const exp = Math.min(14, Math.max(0, age - 20));
  const q = contractQualityScore({ overallRating: ovr, performanceScore: null, gamesPlayed: 0 });
  const s = priceContractCents({ season: SEASON, quality: q, age, yearsOfExperience: exp, position: "SF" });
  const ys = Array.from({ length: 10 }, (_, i) => pickContractLength(q, age, createSeededRandom(`${name}${i}`)));
  console.log(`  ${name.padEnd(24)}${String(ovr).padStart(5)}${String(age).padStart(5)}${usd(s).padStart(10)}${ys.join(",").padStart(20)}`);
}

// --------------------------------------------------------------- 9/10. TOP AND BOTTOM
h("9 & 10. TOP-END AND LOW-END CONTROL");
console.log(`  ${"ARCHETYPE".padEnd(22)}${"OVR".padStart(5)}${"SALARY".padStart(10)}${"%CAP".padStart(8)}${"MAX ALLOWED".padStart(13)}${"AT MAX?".padStart(9)}`);
for (const [name, ovr, age, exp] of [
  ["MVP superstar", 99, 28, 8], ["All-NBA", 93, 28, 8], ["All-Star", 87, 28, 8],
  ["high-end starter", 81, 28, 8], ["average starter", 76, 28, 8], ["rotation", 70, 28, 8],
  ["replacement vet", 63, 33, 12], ["15th man", 60, 25, 4], ["young fringe", 62, 21, 1],
] as [string, number, number, number][]) {
  const q = contractQualityScore({ overallRating: ovr, performanceScore: null, gamesPlayed: 0 });
  const s = priceContractCents({ season: SEASON, quality: q, age, yearsOfExperience: exp, position: "SF" });
  const maxC = cap * maxSalaryFractionFor({ age, yearsOfExperience: exp });
  console.log(`  ${name.padEnd(22)}${String(ovr).padStart(5)}${usd(s).padStart(10)}${pctCap(s).padStart(8)}${usd(maxC).padStart(13)}${(Math.abs(s - maxC) < cap * 0.002 ? "YES" : "no").padStart(9)}`);
}
...
const byPos = (["PG", "SG", "SF", "PF", "C"] as const).map((pos) => ({
  pos, s: priceContractCents({ season: SEASON, quality: q82, age: 27, yearsOfExperience: 7, position: pos }),
}));
const posMin = Math.min(...byPos.map((x) => x.s));
for (const { pos, s } of byPos) console.log(`  ${pos}  ${usd(s).padStart(9)}   ${((s / posMin - 1) * 100).toFixed(1).padStart(6)}% above cheapest`);

// --------------------------------------------------------------- 19. ROOKIE SCALE
h("19. ROOKIE SCALE BY DRAFT SLOT (using expected rating for the slot)");
console.log(`  ${"PICK".padStart(6)}${"EXP OVR".padStart(9)}${"YR1 SALARY".padStart(12)}${"%CAP".padStart(8)}${"VS VET PRICE".padStart(14)}`);
for (const [pick, ovr] of [[1, 72], [5, 71], [14, 70], [30, 67], [45, 65], [60, 62]] as [number, number][]) {
  const q = contractQualityScore({ overallRating: ovr, performanceScore: null, gamesPlayed: 0 });
  const rookie = priceContractCents({ season: SEASON, quality: q, age: 20, yearsOfExperience: 0, position: "SF" });
  const vet = priceContractCents({ season: SEASON, quality: q, age: 27, yearsOfExperience: 7, position: "SF" });
  console.log(`  ${String(pick).padStart(6)}${String(ovr).padStart(9)}${usd(rookie).padStart(12)}${pctCap(rookie).padStart(8)}${((rookie / vet) * 100).toFixed(0).padStart(13)}%`);
}
...
for (const ovr of [70, 74, 78, 79, 80, 81, 82, 86, 90, 94, 98]) {
  const q = contractQualityScore({ overallRating: ovr, performanceScore: null, gamesPlayed: 0 });
  const s = priceContractCents({ season: SEASON, quality: q, age: 27, yearsOfExperience: 7, position: "SF" });
  console.log(`    ovr ${String(ovr).padStart(3)}  ${usd(s).padStart(9)}${prev ? `   ${(((s - prev) / prev) * 100).toFixed(1).padStart(6)}% vs previous` : ""}`);
  prev = s;
}
  ...
  for (const age of [21, 25, 29, 33, 37]) {
    ...
    for (let k = 0; k < rngSeeds; k++) {
      const c = generateContract({
        season: SEASON, overallRating: ovr, performanceScore: null, gamesPlayed: 0,
        age, yearsOfExperience: exp, position: "SF", seed: `g${ovr}${age}${k}`,
      });
      gen.push(Number(c.years[0].salaryCents));
    }
  }
  if (ovr > 60) {
    const a = priceContractCents({ season: SEASON, quality: contractQualityScore({ overallRating: ovr - 1, performanceScore: null, gamesPlayed: 0 }), age: 27, yearsOfExperience: 7, position: "SF" });
    const b = priceContractCents({ season: SEASON, quality: contractQualityScore({ overallRating: ovr, performanceScore: null, gamesPlayed: 0 }), age: 27, yearsOfExperience: 7, position: "SF" });
...
console.log(`  min ${usd(Math.min(...gen))}   median ${usd(pctile(gen, 0.5))}   max ${usd(Math.max(...gen))}`);
...
for (const p of league) {
  const belowMedianQuality = p.ovr < pctile(league.map((x) => x.ovr), 0.5);
  ...
  if (p.ovr < 80 && p.salary > cap * maxSalaryFractionFor({ age: p.age, yearsOfExperience: p.exp }) * 0.95)
...
const drift = league.map((p) => ({ p, d: (ratingRank.get(p.name) ?? 0) - (salaryRank.get(p.name) ?? 0) }));
console.log(`\n  10 MOST OVERPAID (salary rank far above rating rank)`);
for (const { p, d } of [...drift].sort((a, b) => b.d - a.d).slice(0, 10))
  console.log(`    ${p.name.slice(0, 22).padEnd(24)}${String(p.ovr).padStart(4)} ovr  ${usd(p.salary).padStart(9)}  +${d} places`);
