...
import { developPlayerRating, developmentTraitFromId } from "../src/lib/development/developPlayerRating";
...
const BASE = 2026;
const q = (ovr: number) => contractQualityScore({ overallRating: ovr, performanceScore: null, gamesPlayed: 0 });
...
for (const ovr of [72, 82, 90]) {
  ...
  console.log(`  OVR ${ovr}  base ${usd(Number(base)).padStart(9)}  |  ` +
...
console.log(`  ${"TEAM ROOM".padStart(12)}${"PLAYER ASK".padStart(13)}${"WOULD ACCEPT MIN?".padStart(20)}`);
for (const room of [5, 25, 60]) {
  const ask = askFor(85);
  const d = evaluateFreeAgentOffer({
    askingPriceCents: ask, offerSalaryCents: BigInt(veteranMinimumCents(BASE, 7)),
    rivalSuitors: 0, valueTier: getPlayerValueTier(85), minimumSalaryCents: veteranMinimumCents(BASE, 7),
  });
  console.log(`  ${(`$${room}M`).padStart(12)}${usd(Number(ask)).padStart(13)}${(d.accepted ? "YES" : "no").padStart(20)}`);
}
...
const rivals: RivalTeam[] = Array.from({ length: 29 }, (_, i) => ({
  leagueTeamId: `T${i}`, abbreviation: `T${i}`,
  capSpaceCents: BigInt(Math.round(60_000_000_00 * (i / 28))),
  needs: computeTeamNeeds([]), rosterCount: 12,
}));
for (const ovr of [78, 88]) {
  const ask = askFor(ovr);
  const interest = computeRivalInterest({ position: "SF", overallRating: ovr, estimatedValueCents: ask }, rivals);
  const final = demandAdjustedPriceCents(ask, interest.rivals.length, 27, BASE, 7);
  console.log(`  OVR ${ovr}: ask ${usd(Number(ask))}, ${interest.rivals.length} of 29 clubs can afford him, final ${usd(Number(final))} (+${(((Number(final) / Number(ask)) - 1) * 100).toFixed(0)}%)`);
}
...
const probe = (label: string, ovr: number, exp: number, offerCents: number, suitors: number) => {
  ...
  const d = evaluateFreeAgentOffer({
    askingPriceCents: ask, offerSalaryCents: BigInt(offerCents), rivalSuitors: suitors,
    valueTier: getPlayerValueTier(ovr), minimumSalaryCents: veteranMinimumCents(BASE, exp),
  });
  const legal = validateSigning({
    season: BASE, offerSalaryCents: BigInt(offerCents), yearsOfExperience: exp,
    team: { apronLevel: ApronLevel.SECOND_APRON, capSpaceCents: 0n },
  });
  console.log(`  ${label.padEnd(46)}${(legal.isValid ? "legal" : "illegal").padStart(9)}${(d.accepted ? "  ACCEPTS" : "  refuses").padStart(10)}   he wants ${usd(Number(d.requiredSalaryCents))}`);
};
...
console.log(`  ${"SEASON".padStart(8)}${"CAP".padStart(10)}${"MAX (35%)".padStart(12)}${"MIN (10y)".padStart(12)}${"OVR-82 PRICE".padStart(14)}${"PRICE/CAP".padStart(11)}`);
for (const offset of [0, 2, 4, 9]) {
  ...
  console.log(`  ${String(season).padStart(8)}${usd(c).padStart(10)}${usd(c * 0.35).padStart(12)}${usd(Number(veteranMinimumCents(season, 10))).padStart(12)}${usd(p82).padStart(14)}${((p82 / c) * 100).toFixed(1).padStart(10)}%`);
}
...
interface Row { fullName: string; teamAbbreviation: string | null; position: string; seedOverallRating: number | null; birthDate?: string | null; draftYear?: number | null }
const ds = JSON.parse(fs.readFileSync(path.join(__dirname, "..", "prisma", "data", "nbaDataset.json"), "utf8")) as { players: Row[] };
const { rostered } = selectTopPerTeam<Row>(ds.players, (p) => p.teamAbbreviation, (p) => p.seedOverallRating ?? 0, DEFAULT_MAX_ROSTER_SIZE);
...
for (const p of ds.players) {
  ...
  const s = priceContractCents({
    season: BASE, quality: q(p.seedOverallRating), age: resolvePlayerAge(src, BASE),
    yearsOfExperience: resolvePlayerExperience(src, BASE), position: p.position,
  });
...
console.log(`  cap ${usd(Number(rules.salaryCapCents))}  tax ${usd(Number(rules.luxuryTaxCents))}  1st apron ${usd(Number(rules.firstApronCents))}  2nd apron ${usd(Number(rules.secondApronCents))}\n`);
...
interface P { id: string; overall: number; potential: number; age: number }
const start: P[] = [];
for (const p of ds.players) {
  if (!rostered.has(p) || p.seedOverallRating == null) continue;
  const src = { birthDate: p.birthDate ? new Date(p.birthDate) : null, draftYear: p.draftYear ?? null };
  start.push({ id: p.fullName, overall: p.seedOverallRating, potential: p.seedOverallRating, age: resolvePlayerAge(src, BASE) });
}
const rng = makeRng(20260816);
let pool = start.map((p) => ({ ...p }));
console.log(`  ${"SEASON".padStart(8)}${"CAP".padStart(10)}${">$10M".padStart(8)}${">$20M".padStart(8)}${">$30M".padStart(8)}${">$40M".padStart(8)}${"MEDIAN".padStart(10)}${"MED/CAP".padStart(9)}`);
for (let season = 0; season <= 10; season++) {
  if (season > 0) {
    for (const p of pool) {
      p.overall = developPlayerRating({ overallRating: p.overall, potentialRating: p.potential, age: p.age, rng, developmentTrait: developmentTraitFromId(p.id) });
      p.age += 1;
    }
    pool = pool.filter((p) => !shouldRetire(p.age, p.overall, rng));
    for (const [i, pr] of generateDraftClass(rng).prospects.entries())
      pool.push({ id: `c${season}p${i}`, overall: pr.overallRating, potential: pr.potentialRating, age: pr.age });
  ...
  if (season === 0 || season === 2 || season === 4 || season === 9 || season === 10) {
    ...
    const ss = pool.map((p) => priceContractCents({ season: yr, quality: q(p.overall), age: p.age, yearsOfExperience: Math.max(0, p.age - 20), position: "SF" }));
    const med = [...ss].sort((a, b) => a - b)[Math.floor(ss.length / 2)];
    const over = (t: number) => ss.filter((v) => v > t * 1e8).length;
    console.log(`  ${String(yr).padStart(8)}${usd(c).padStart(10)}${String(over(10)).padStart(8)}${String(over(20)).padStart(8)}${String(over(30)).padStart(8)}${String(over(40)).padStart(8)}${usd(med).padStart(10)}${((med / c) * 100).toFixed(1).padStart(8)}%`);
  }
}
