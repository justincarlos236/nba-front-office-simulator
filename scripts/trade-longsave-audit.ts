...
function salaryFor(rating: number, age: number, exp: number, position: string, season: number): bigint {
  return BigInt(priceContractCents({
    season, quality: contractQualityScore({ overallRating: rating, performanceScore: null, gamesPlayed: 0 }),
    age, yearsOfExperience: exp, position,
  }));
}
...
function seedLeague(): Map<string, CpuRosterPlayer[]> {
  ...
  for (const p of ds.players) {
    ...
    m.set(p.teamAbbreviation, [...(m.get(p.teamAbbreviation) ?? []), {
      leaguePlayerId: `${p.fullName}`, playerName: p.fullName,
      ...
      salaryCents: salaryFor(p.seedOverallRating, age, exp, pos, BASE), noTradeClause: false,
    }]);
  }
  return m;
}

interface Snapshot { season: number; strengthSd: number; best: number; worst: number; topTeamStars: number; hhi: number }
...
function run(tradingOn: boolean, seed: number): Snapshot[] {
  ...
  const snapshot = (season: number) => {
    ...
    snaps.push({
      season, strengthSd: sd(strengths), best: Math.max(...strengths), worst: Math.min(...strengths),
      topTeamStars: Math.max(...starsPerTeam), hhi,
    });
  };
  snapshot(0);
  ...
  for (let s = 1; s <= SEASONS; s++) {
    ...
    if (tradingOn) {
      const order = [...league.entries()].map(([t, r]) => ({ t, s: computeTeamStrength(r.map((p) => p.rating)) }))
        .sort((a, b) => a.s - b.s);
      const teams: CpuTeam[] = order.map(({ t }, i) => {
        const roster = league.get(t)!;
        const avgAge = roster.reduce((a, p) => a + p.age, 0) / roster.length;
        return {
          leagueTeamId: t, teamLabel: t, roster,
          capState: {
            apronLevel: getApronLevel(roster.reduce((a, p) => a + p.salaryCents, 0n), getSeasonCapRules(season)),
            capSpaceCents: computeCapSheet({
              season, contracts: roster.map((p) => ({ playerId: p.leaguePlayerId, salaryCents: p.salaryCents })),
            }).capSpaceCents,
    ...
    for (const [t, roster] of league) {
      ...
      for (const p of roster) {
        p.rating = developPlayerRating({
          overallRating: p.rating, potentialRating: p.potentialRating, age: p.age, rng,
    ...
    draftOrder.forEach(({ t }, i) => {
      ...
      for (const pick of [prospects[i], prospects[i + 30]]) {
        ...
        roster.push({
          leaguePlayerId: `s${s}t${t}p${i}`, playerName: `Rookie ${i}`,
          rating: pick.overallRating, potentialRating: pick.potentialRating, age: pick.age,
          position: pos as (typeof POSITIONS)[number], injuryStatus: "HEALTHY",
          careerGamesMissedToInjury: 0,
          salaryCents: salaryFor(pick.overallRating, pick.age, 0, pos, season), noTradeClause: false,
        });
      }
...
console.log(`${"SEASON".padStart(8)}${"STRENGTH SD".padStart(24)}${"MOST STARS ON ONE TEAM".padStart(26)}${"STAR CONCENTRATION".padStart(22)}`);
console.log(`${"".padStart(8)}${"trades / no trades".padStart(24)}${"trades / no trades".padStart(26)}${"trades / no trades".padStart(22)}`);
for (let i = 0; i <= SEASONS; i++) {
  if (i !== 0 && i !== 4 && i !== 8 && i !== SEASONS) continue;
  const f = (runs: Snapshot[][], pick: (s: Snapshot) => number, d = 2) => meanAt(runs, i, pick).toFixed(d);
  console.log(
    `${String(i).padStart(8)}` +
